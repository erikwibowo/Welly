...
export default function Index({ title, filters, permissions }: { title: string; filters: Filter; permissions: App.Paginate<App.Models.Permission> }) {
    ...
    return (
        <TableLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="mx-auto w-full space-y-4 py-4">
                            ...
                            <DropdownMenu modal={false}>
                                ...
                                <DropdownMenuContent align="start">
                                    <DeleteBulk
                                        ...
                                        routes="permission.destroy-bulk"
                                        id={selected}
                                    />
                                </DropdownMenuContent>
                            </DropdownMenu>
                ...
                <Empty show={permissions.data.length === 0} />
                <Table className={permissions.data.length > 0 ? 'w-full' : 'hidden'}>
                    <TableHeader>
                        <TableRow>
                            <TableHead>
                                <Checkbox
                                    checked={selected.length > 0 && selected.length === permissions.data.length}
                                    onCheckedChange={(value) => setSelected(value ? permissions.data.map((permission) => permission.id) : [])}
                                />
                            </TableHead>
                            <TableHead>#</TableHead>
                    ...
                    <TableBody>
                        {permissions.data.map((permission, index) => (
                            <TableRow key={permission.id} data-state={selected.includes(permission.id) ? 'selected' : ''}>
                                <TableCell>
                                    <Checkbox
                                        checked={selected.includes(permission.id)}
                                        onCheckedChange={(value) => {
                                            setSelected((prev) => (value ? [...prev, permission.id] : prev.filter((id) => id !== permission.id)));
                                        }}
                                    />
                                </TableCell>
                                <TableHead>{numberFormat(permissions.from + index)}</TableHead>
                                <TableCell className="font-medium">{permission.name}</TableCell>
                                <TableCell>{permission.guard_name}</TableCell>
                                <TableCell>{dateFormat(permission.created_at || '')}</TableCell>
                                <TableCell className="text-right">
                                    <DropdownMenu modal={false}>
                                        ...
                                        <DropdownMenuContent align="end">
                                            <DropdownMenuLabel>
                                                <p className="max-w-40 truncate font-semibold">{permission.name}</p>
                                            </DropdownMenuLabel>
                                            <Separator className="my-1" />
                                            <Edit title={title} permission={permission} />
                                            <Delete
                                                title={title}
                                                permissions={['permission delete']}
                                                routes="permission.destroy"
