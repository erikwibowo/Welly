...
import { EyeIcon, ClipboardCopyIcon, DownloadIcon, EllipsisIcon, Link2Icon } from 'lucide-react';
...
export const ActionWrapper = React.memo(({ children, className, ...props }: React.ComponentProps<'div'>) => (
    <div
        className={cn(
...
export const ActionButton = React.memo(({ icon, tooltip, className, ...props }: ActionButtonProps) => (
    <Tooltip>
...
const ActionItems: Array<{
    ...
    {
        key: 'onCopy',
        icon: <ClipboardCopyIcon className="icon" />,
        tooltip: 'Copy image to clipboard',
    },
    {
        key: 'onCopyLink',
        icon: <Link2Icon className="icon" />,
            ...
            {shouldMerge ? (
                <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
                    <DropdownMenuTrigger asChild>
                        <ActionButton icon={<EllipsisIcon className="icon" />} tooltip="Open menu" onClick={(e) => e.preventDefault()} />
                    </DropdownMenuTrigger>
