...
export const ImageViewBlock: React.FC<NodeViewProps> = ({ editor, node, selected, updateAttributes }) => {
    ...
    const containerMaxWidth = containerRef.current ? parseFloat(getComputedStyle(containerRef.current).getPropertyValue('--editor-width')) : Infinity;
            ...
            >
                <div
                    className={cn('relative flex h-full cursor-default flex-col items-center gap-2 rounded', {
                        'outline-primary outline-2 outline-offset-1': selected || isResizing,
                    })}
                >
                    ...
                    {imageState.error && (
                        <ActionWrapper>
                            <ActionButton icon={<TrashIcon className="icon" />} tooltip="Remove image" onClick={onRemoveImg} />
                        </ActionWrapper>
                    )}
